@extends('layouts.app',['title'=>' |  Mix Item: '.$mix->name])
                ...
                <div class="col-sm-12">
                    <div class="page-title-box">
                        ...
                        <h4 class="page-title">
                            ...
                            <span>Edit Mix:
                            <a href="{{ route('mixes.show',$mix->id) }}" class="text-primary">{{ $mix->name }}</a>
                            </span>
                        </h4>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    {{ Form::model($mix,['route'=>['mixes.update',$mix->id],'method'=>'POST',
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6 {{ $errors->has('created_at') ? 'has-error' :'' }}">
                                                ...
                                                {{ Form::date('created_at',$mix->created_at,['class'=>'form-control']) }}
                                        ...
                                        @for($i=1;$i<=5;$i++)
                                            <div class="row">
                                                <div class="form-group col-md-6 {{ $errors->has('item_'.$i) ? 'has-error' :'' }}">
                                                    <label for="item_{{$i}}"> Select Item {{$i}}</label>
                                                    {{ Form::select('item_'.$i, $items,isset($mix->items[$i-1]->id) ? $mix->items[$i-1]->id : null ,['class'=>'form-control select2']) }}

                                                    @if($errors->has('item_'.$i))
                                                        <span class="text-danger">{{ $errors->first('item_'.$i) }}</span>
                                                    @endif
                                                </div>

                                                <div class="form-group col-md-6 {{ $errors->has('percentage_'.$i) ? 'has-error' :'' }}">
                                                    <label for="percentage_{{$i}}"> Insert Item {{ $i }} Percentage</label>
                                                    <div class="input-group bootstrap-touchspin">
                                                    <span class="input-group-btn">
                                                        <button class="btn btn-default bootstrap-touchspin-down" type="button">-</button>
                                                    </span>
                                                        <span class="input-group-addon bootstrap-touchspin-prefix" style="display: none;"></span>
                                                        {{ Form::text('percentage_'.$i,isset($mix->items[$i-1]->pivot->percentage) ? $mix->items[$i-1]->pivot->percentage : null,['id'=>'percentage_'.$i,'class'=>'form-control']) }}
                                                        <span class="input-group-addon bootstrap-touchspin-postfix">%</span>
                                                        <span class="input-group-btn">
                                                        <button class="btn btn-default bootstrap-touchspin-up" type="button">+</button>
                                                    </span>
                                                    </div>
                                                    @if($errors->has('percentage_'.$i))
                                                        <span class="text-danger">{{ $errors->first('percentage_'.$i) }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        @endfor
